...
type CohortController struct {
	service *services.CohortService
}

func NewCohortController(service *services.CohortService) *CohortController {
	return &CohortController{service: service}
}

func (c *CohortController) GetAllCohorts(w http.ResponseWriter, r *http.Request) {
	cohorts, err := c.service.FetchAllCohorts()
	if err != nil {
		http.Error(w, "Failed to fetch cohorts", http.StatusInternalServerError)
		return
	}
...
func (c *CohortController) GetCohortByID(w http.ResponseWriter, r *http.Request) {
	idStr := mux.Vars(r)["id"]
	id, err := strconv.Atoi(idStr)
	if err != nil {
		http.Error(w, "Invalid ID", http.StatusBadRequest)
	...
	cohort, err := c.service.FetchCohortByID(uint(id))
	if err != nil {
		http.Error(w, "Cohort not found", http.StatusNotFound)
		return
	}
...
func (c *CohortController) CreateCohort(w http.ResponseWriter, r *http.Request) {
	var cohort models.Cohort
	if err := json.NewDecoder(r.Body).Decode(&cohort); err != nil {
		http.Error(w, "Invalid JSON", http.StatusBadRequest)
		return
	...
	created, err := c.service.CreateCohort(cohort)
	if err != nil {
		http.Error(w, "Failed to create cohort", http.StatusInternalServerError)
		return
	}
